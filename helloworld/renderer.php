...
/**
 * Renderer for displaying mod_helloworld as HTML
 *
 * @package    mod_helloworld
 * @copyright  2020 Jakub Kleban <jakub.kleban2000@gmail.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class mod_helloworld_renderer extends plugin_renderer_base {
    ...
    private function message_input($url) {
        ...
        $output .= html_writer::start_tag('form', array(
            ...
            $output .= html_writer::start_div('form-group');
                $output .= html_writer::label(get_string('typemessage', 'mod_helloworld'), 'messagetextarea');
                //$output .= html_writer::label('SQL Select:', 'messagetextarea');
    ...
    private function display_messages(string $url) {
        ...
        $sql = "SELECT m.id, m.message, m.timecreated, u.id AS userid, $userfields
                FROM {mod_helloworld_messages} m
        ...
        foreach ($messages as $message) {
            ...
            $lastupdated = get_string('lastupdated', 'mod_helloworld', $formatedtime);
            $id = $message->id;
        ...
        //$output .= var_dump(context_system::instance());
